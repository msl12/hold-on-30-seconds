var game = {
    ...
    oTime: function() {
        ...
        var show = game.$id("p1");
        if (game.sec < 10)
            show.innerHTML = game.hour + ":" + game.minute + ":0" + game.sec + ":" + game.ms + "";
        if (game.minute < 10)
            show.innerHTML = game.hour + ":0" + game.minute + ":" + game.sec + ":" + game.ms + "";
        if (game.sec < 10 && game.minute < 10)
            show.innerHTML = game.hour + ":0" + game.minute + ":0" + game.sec + ":" + game.ms + "";
        else
            show.innerHTML = game.hour + ":" + game.minute + ":" + game.sec + ":" + game.ms + "";
    },
    iMouse: null,
    oDiv: null,
    ...
    doMove: function(ev) {
        ...
        if (l < game.writeDiv().offsetLeft || l > (game.writeDiv().offsetWidth - game.redDiv().offsetWidth + game.writeDiv().offsetLeft) || t < game.writeDiv().offsetTop || t > (game.writeDiv().offsetHeight - game.redDiv().offsetHeight + game.writeDiv().offsetTop)) {
            game.clearSet();
            clearInterval(game.timeStart);
            game.timeStart = null;
            game.stopMove();
    ...
    crashCheck: function(objDrag, oDiv) {
        ...
        if (dragDiv1.r1 < moveDiv2.l1 || dragDiv1.l1 > moveDiv2.r1 || dragDiv1.b1 < moveDiv2.t1 || dragDiv1.t1 > moveDiv2.b1) {
            return false;
        } else {
            return true;
        }
